import type { AnswerLetter, Puzzle, FlatPuzzle, ValidationRule } from "./types.ts";
import {
	LETTERS, L2I, NONE, letterIdx, flattenPuzzle,
	RT_COUNT_ANSWER, RT_COUNT_ANSWER_BEFORE, RT_COUNT_ANSWER_AFTER,
	RT_COUNT_VOWEL, RT_COUNT_CONSONANT, RT_MOST_COMMON_COUNT,
	RT_CLOSEST_AFTER, RT_CLOSEST_BEFORE, RT_FIRST_WITH, RT_LAST_WITH,
	RT_ANSWER_OF, RT_LETTER_DIST, RT_UNIQUE, RT_SAME_AS,
} from "./types.ts";
...
function findContradiction(
	...
	for (let qi = 0; qi < n; qi++) {
		...
		if (msg) {
			...
			return hintSteps(
				qi,
				`Reconsider ${Q(qi)}'s answer (${answer}).`,
				msg,
				{ type: "contradiction", questionIndex: qi },
			);
		}
	}
	return null;
...
function explainContradiction(
	...
	if (rule.type === "most_common_count") {
		const counts = LETTERS.map(
			(l) => countMatching(answers, (a) => a === l, 0, n).count,
		);
...
export function findActionFast(
	...
	const fp = _fpCache.p === puzzle ? _fpCache.fp : (_fpCache.p = puzzle, _fpCache.fp = flattenPuzzle(puzzle));
	return findActionFp(fp, answers, markSets, n);
}

// Cache FlatPuzzle to avoid re-flattening on every call for the same puzzle
const _fpCache: { p: Puzzle | null; fp: FlatPuzzle } = { p: null, fp: undefined! };
...
function findActionFp(
	...
	for (let qi = 0; qi < n; qi++) {
		...
		if (r.t === RT_COUNT_ANSWER || r.t === RT_COUNT_ANSWER_BEFORE || r.t === RT_COUNT_ANSWER_AFTER ||
			r.t === RT_COUNT_VOWEL || r.t === RT_COUNT_CONSONANT) {
			if (on === on) { // not NaN
		...
		if (r.t === RT_UNIQUE) {
			if (countMatchingSimple(answers, a, 0, n) > 1) return { type: "contradiction", questionIndex: qi };
		}
	...
	for (let qi = 0; qi < n; qi++) {
		...
		for (let oi = 0; oi < 5; oi++) {
			...
			if ((r.t === RT_CLOSEST_AFTER || r.t === RT_FIRST_WITH) && on !== NONE) {
				...
				for (let j = afterIdx + 1; j < pos; j++) {
					if (answers[j] === r.answer) return { type: "eliminate", questionIndex: qi, optionIndex: oi };
				}
			}
...
function countPred2(r: { t: number; answer: string | null }): ((a: AnswerLetter) => boolean) | null {
	switch (r.t) {
...
function countRange2(r: { t: number; beforeIndex: number; afterIndex: number }, n: number): [number, number] {
...
function traceAssumption(
	...
	if (scratchAns.length !== n) {
		...
		scratchMarks = Array.from({ length: n }, () => ["unmarked", "unmarked", "unmarked", "unmarked", "unmarked"] as Marks);
	}
	for (let i = 0; i < n; i++) {
	...
	for (let iter = 0; iter < n * 5; iter++) {
		...
		if (action.type === "contradiction") {
			...
			const chain = chainSteps.length > 0
				? chainSteps.join(", then ") + " — but "
				: "";
			return {
