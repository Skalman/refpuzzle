...
for (const [fromKey, toKey] of edges) {
	...
	debug.push(
		`    <circle cx="${r(tipX)}" cy="${r(tipY)}" r="2" fill="lime" opacity="0.8"/>`,
	);
	...
	console.log(`${fromKey}→${toKey}: sweep=${baseSweep} dir=${sign > 0 ? "CCW" : "CW"} tangent=${r(deg)}°`);
}

const dotsSvg = Object.values(nodes)
	.map((n) => `    <circle cx="${n.x}" cy="${n.y}" r="${DOT_R}"/>`)
	.join("\n");

const STYLE = `<style>@media (max-width: 47px) { .heads { display: none; } .arcs { stroke-width: 4.5; } }</style>`;
const ARCS_SVG = arcs.join("\n");
const HEADS_SVG = heads.join("\n");
...
// 0. Original (the reference)
const original = makeSvg(`  <rect width="100" height="100" rx="20" fill="#6366f1"/>
  <g stroke="white" stroke-width="${STROKE_W}" stroke-linecap="round" fill="none" class="arcs">
${ARCS_SVG}
  </g>
  <g fill="white" class="heads">
${HEADS_SVG}
  </g>
  <g fill="white">
${dotsSvg}
...
function buildHeads(color?: string[]): string {
	return heads
		.map((h, i) => {
			const mid = tipMids[i];
			let s = h.replace("<polygon", `<polygon style="--d:${HEAD_DELAYS[i]}s; transform-origin:${r(mid.x)}px ${r(mid.y)}px"`);
...
// 4. Animated arcs — each head appears as its arc finishes
const v4 = makeSvg(`  <style>
    @media (max-width: 47px) { .heads { display: none; } .arcs { stroke-width: 4.5; } }
    .arcs path { stroke-dasharray: 80; stroke-dashoffset: 80; animation: draw 1.0s ease-out var(--d) forwards; }
    @keyframes draw { to { stroke-dashoffset: 0; } }
    .heads polygon { opacity: 0; transform: scale(0); animation: pophead 0.25s ease-out var(--d) forwards; }
...
// 6. Colored arcs on purple bg
const coloredArcsSvg = arcs
	.map((a, i) => a.replace("/>", ` stroke="${arcColors[i]}"/>`))
	.join("\n");
const coloredHeadsSvg = heads
	.map((h, i) => h.replace("/>", ` fill="${arcColors[i]}"/>`))
	.join("\n");
const coloredDotsSvg = nodeList
	.map((n, i) => `    <circle cx="${n.x}" cy="${n.y}" r="${DOT_R}" fill="${nodeColors[i]}"/>`)
	.join("\n");
const v6 = makeSvg(`  <rect width="100" height="100" rx="20" fill="#6366f1"/>
...
// 8. Pulsing dots
const v8 = makeSvg(`  <style>
    @media (max-width: 47px) { .heads { display: none; } .arcs { stroke-width: 4.5; } }
    .dots circle { animation: pulse 2s ease-in-out 2; animation-delay: var(--d); }
    .dots.no-anim circle { animation: none; }
    .dots.once circle { animation-iteration-count: 1; }
...
const extendedArcsSvg = tipMids
	.map((p, i) => {
		const from = Object.values(nodes)[edges[i][0] === "a" ? 0 : edges[i][0] === "b" ? 1 : edges[i][0] === "c" ? 2 : 3];
		return `    <path d="M${r(from.x)},${r(from.y)} A${ARC_R},${ARC_R} 0 0,${p.sweepFlag} ${r(p.x)},${r(p.y)}"/>`;
	})
	.join("\n");
const v11 = makeSvg(`  <style>
    .tip-circles { display: none; }
    .arcs-ext { display: none; }
    @media (max-width: 47px) { .heads { display: none; } .arcs { display: none; } .arcs-ext { display: block; stroke-width: 8; } .tip-circles { display: block; } }
  </style>
  <rect width="100" height="100" rx="20" fill="#6366f1"/>
  <g stroke="white" stroke-width="${STROKE_W}" stroke-linecap="round" fill="none" class="arcs">
${ARCS_SVG}
...
writeFileSync(resolve(__dirname, "../public/logo.svg"), v12);
