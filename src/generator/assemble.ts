...
function tryGenerate(
	...
	let solution: AnswerLetter[] = Array.from({ length: n }, () =>
		rng.pick(LETTERS),
	);
...
type HintMarks = [OptionMark, OptionMark, OptionMark, OptionMark, OptionMark];

function checkSolvable(puzzle: Puzzle, n: number): boolean {
	const marks: HintMarks[] = Array.from({ length: n }, () =>
		["unmarked", "unmarked", "unmarked", "unmarked", "unmarked"] as HintMarks,
	);
	...
	switch (rule.type) {
		...
		case "equal_count_as": {
			...
			for (const l of LETTERS) {
				if (l !== rule.answer && countLetter(sol, l) === refCount)
					matches++;
			}
			return matches >= 1;
		}
		...
		switch (rule.type) {
			...
			case "equal_count_as": {
				...
				const hasMatch = LETTERS.some(
					(l) => l !== rule.answer && countLetter(sol, l) === refCount,
				);
				if (hasMatch) break;
...
function computeConstrainedAnswer(
	...
	switch (rule.type) {
		...
		case "equal_count_as": {
			...
			for (const l of LETTERS) {
				if (l !== rule.answer && countLetter(solution, l) === refCount)
					return l;
			}
			return null;
		}
...
function computeValue(
	...
	switch (rule.type) {
		...
		case "letter_distance":
			return String(
				Math.abs(L2I[sol[qi]] - L2I[sol[rule.otherQuestionIndex]]),
			);
		default:
			return "";
	}
...
function positionalDistractors(correct: string, n: number, rng: RNG, rule: ValidationRule): string[] {
	...
	if (rule.type === "count_answer_after") { minPos = rule.afterIndex + 2; }
	if (rule.type === "count_answer_before") { maxPos = rule.beforeIndex; }
