...
const levels = targetLevel ? profiles.filter((p) => p.level === targetLevel) : profiles;
...
for (const profile of levels) {
  ...
  for (let pi = 0; pi < count; pi++) {
    ...
    while (!result && attempts < maxOuter) {
      const seed = baseSeed + profile.level * 10000 + seedOffset * 7919;
      seedOffset++;
    ...
    if (!result) {
      console.error(`  FAILED puzzle ${pi + 1}/${count} after ${attempts} seeds`);
      allOk = false;
      break;
    }
  ...
  console.log(`  OK  ${elapsed}s  → ${filename}  (${puzzles.length} puzzles)`);
}
  ...
  const qs = puzzle.questions
    .map((q) => {
      const opts = q.options
        ...
        .join(",\n");
      return `\t\t{\n\t\t\toptions: [\n${opts},\n\t\t\t],\n\t\t\trule: ${JSON.stringify(q.questionType)},\n\t\t}`;
    })
    .join(",\n");
