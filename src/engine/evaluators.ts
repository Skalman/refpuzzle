...
import {
	LETTERS, NONE, letterIdx, flattenPuzzle,
	...
	RT_PREV_SAME, RT_NEXT_SAME, RT_ONLY_SAME, RT_SAME_AS,
	RT_ONLY_ODD, RT_CONSEC_IDENT, RT_ANSWER_OF,
	RT_LEAST_COMMON, RT_MOST_COMMON, RT_UNIQUE, RT_EQUAL_COUNT,
	RT_SELF, RT_LETTER_DIST, RT_TRUE_STMT,
} from "./types.ts";
...
// High-level evaluate for UI (accepts Puzzle, converts internally)
export function evaluate(
	...
	switch (r.t) {
		...
		case RT_ONLY_SAME: {
			...
			for (let i = 0; i < n; i++) {
				if (i !== questionIdx && answers[i] === selectedAnswer)
					matches.push(i);
			}
		...
		case RT_SAME_AS: {
			...
			if (targetQ < 0 || targetQ >= n || answers[targetQ] == null)
				return false;
		...
		case RT_TRUE_STMT: {
			...
			for (let i = 0; i < 5; i++) {
				...
				if (isTrue) trueCount++;
				if (LETTERS[i] === selectedAnswer && isTrue)
					selectedIsTrue = true;
			}
...
// Convenience wrapper that accepts Puzzle (for UI code)
export function evaluateWithPuzzle(
	rule: FlatRule,
	questionIdx: number,
	selectedAnswer: AnswerLetter,
	answers: (AnswerLetter | null)[],
	...
	return evaluate(rule, questionIdx, selectedAnswer, answers, flattenPuzzle(puzzle));
}
